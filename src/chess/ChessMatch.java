...
public class ChessMatch {
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> listFilterCheckMate = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
		
		for(Piece p: listFilterCheckMate) {
			boolean[][] mat = p.possibleMoves();
			for (int i = 0; i < board.getRows();i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if(mat[i][j]) {
